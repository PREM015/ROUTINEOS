 ...
 * named component and as the default export consumed by
 * `src/lib/email/templates.ts`. `renderAchievementUnlocked` serializes the
                  ...
                  <tbody>
                    <tr>
                      <td style={{ backgroundColor: EMAIL_CARD, borderRadius: '12px', padding: '32px' }}>
                        ...
                        {achievementDescription && achievementDescription.trim().length > 0 && (
                          <p style={{ margin: '0 0 8px', fontSize: '15px', lineHeight: 1.7, color: '#334155' }}>
...
export default AchievementUnlocked as unknown as ComponentType<Record<string, unknown>>;

/**
 * Serialize the achievement unlocked template to a static HTML string.
 */
export function renderAchievementUnlocked(
  props: AchievementUnlockedProps = {}
): string {
  return renderToStaticMarkup(<AchievementUnlocked {...props} />);
}